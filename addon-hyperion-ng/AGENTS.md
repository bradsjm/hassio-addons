...
- `config.json` defines the 2.2.1 add-on metadata for `amd64` and `aarch64`.
...
- When updating the version, verify both upstream `Linux-amd64` and `Linux-arm64` archives. The manual `update-hyperion-version.sh` helper validates them before changing `config.json`.
